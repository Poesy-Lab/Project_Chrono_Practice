...
| 1학기 Week 7~10 (5월) | Phase 3: 차량과 지형 | Chrono::Vehicle 모듈 (차량 + 지형 + 타이어) | 📋 예정 |
| 1학기 Week 11~14 (6월) | Phase 4: 개인 로버 제작 | 각자 이름 폴더에서 1인 1로버 설계/구현 | 📋 예정 |
| 2학기 (9~12월) | Phase 5: 다중 로버 실험 | 여러 환경에서 로버 4대 충돌/상호작용, 결과 출력 | 📋 예정 |
...
7. ```powershell
   ...
   ```
8. 이제 기도하고 기다리십쇼, irrlicht 에러 뜨면 한참 후에 뜨는거라 할 수 있는게 없어요. 팝콘 가져오시면 됩니다
9. 만약 중간에 빨간 에러 없이 installing주르륵 뜨면 완료 된겁니다, 축하드립니다 고비는 넘겼어요
10. 예제를 실행해 봅시다, step1에서 다운로드 한 파일 속에서(C:\Users\%USERNAME%\Documents\irrlicht-1.8.5\bin\Win64-visualStudio) "Irrlicht.dll"라는 파일을 복사+붙여넣기 하여 데모 폴더에 넣어주면 됩니다(C:\Users\%USERNAME%\Documents\chrono_build\bin\Release)
11. 똑같이 Irrlicht.dll을 여기 폴더에도 넣어줍시다(C:\Users\%USERNAME%\anaconda3\envs\chrono\Lib\site-packages\pychrono)
12. Irr로 시작하는 아무 데모나 시작해 봅시다 (demo_IRR_paths가 예뻐요ㅎ)
13. 파이썬 환경(pychrono)이 실행될 때 Irrlicht.dll과 Chrono 엔진 파일들을 자동으로 찾아가게 만들어야 합니다.
14. 윈도우 검색창에 계정의 환경 변수 편집 또는 시스템 환경 변수 편집을 입력해 실행합니다.
15. 하단의 [환경 변수] 버튼을 누릅니다.
16. [사용자 변수] 또는 [시스템 변수] 목록에서 **Path**를 찾아 선택하고 **[편집]**을 누릅니다.
17. [새로 만들기]를 눌러 아래 두 경로를 추가하세요
18. C:\Users\%USERNAME%\Documents\chrono_install\bin
19. C:\Users\%USERNAME%\Documents\irrlicht-1.8.5\bin\Win64-visualStudio
20. 확인을 눌러 모든 창을 닫습니다.
21. 끝!(아마도)
...
### Phase 3: 차량과 지형 (Week 7~10) - "Chrono::Vehicle 모듈 활용"

> **핵심 모듈**: Chrono::Vehicle (지형 클래스 포함) — 캡스톤의 시뮬레이션 레이어 핵심
> 참고: RigidTerrain, SCMTerrain 등 지형 클래스는 별도 모듈이 아니라 Vehicle 모듈(`chrono::vehicle` 네임스페이스)에 포함되어 있습니다.
> Phase 2에서 배운 조인트/모터/스프링이 서스펜션, 타이어, 조향 시스템의 기반입니다.

| 순서 | 주제 | 배우는 것 | 참고 데모 |
|------|------|-----------|-----------|
| 13 | Vehicle 입문: HMMWV | Chrono::Vehicle 구조, 내장 HMMWV 모델 구동, 키보드 조작 | `demo_VEH_HMMWV.py` |
| 14 | RigidTerrain | 평면/메시/높이맵 패치 지형 생성, 차량과 연동 | `demo_VEH_RigidTerrain.py` |
| 15 | 타이어와 서스펜션 | 타이어 모델(Rigid, TMeasy), 서스펜션 유형, 힘/토크 데이터 수집 | `demo_VEH_SuspensionTestRig.py` |
| 16 | 조향과 경로 추종 | PID 조향 컨트롤러, 경로 추종(path-follower), 원형 주행 | `demo_VEH_SteeringController.py` |
| 17 | JSON 차량 커스터마이징 | JSON 파일로 차량 파라미터 변경, 다양한 차량 모델(Sedan, Gator) | `demo_VEH_WheeledJSON.py` |
| 18 | SCMTerrain (변형 토양) | 변형 가능 지형(토양 파라미터), 바퀴 자국 생성, 힘 해석 | `demo_VEH_DeformableSoil.py` |

> **Phase 3 → Phase 4 연결**: Phase 3에서 Vehicle/Terrain 사용법을 익힌 뒤,
> Phase 4에서 각자의 개인 로버를 만들고 Phase 5에서 여러 환경에 함께 배치합니다.

### Phase 4: 개인 로버 제작 (Week 11~14) - "1인 1로버"

> Phase 4에서는 공통 예제를 하나씩 따라가기보다, 각자 이름 폴더에서 자기 로버를 작은 MVP 단위로 만듭니다.
> 폴더 규칙: `lessons/phase4/<name>/`

| 순서 | 주제 | 배우는 것 | 산출물 |
|------|------|-----------|--------|
| 19 | 로버 MVP0 | 단일 차체 강체, 충돌 형상, 초기 속도, CSV 로그 | 충돌 가능한 최소 로버 |
| 20 | 로버 외형/파라미터 | 크기, 질량, 바퀴/트랙 시각 형상, 설정값 분리 | 개인 로버 설계안 |
| 21 | 로버 구동 | 모터/힘 입력, 조향 또는 좌우 차동 구동 | 움직이는 로버 |
| 22 | 로버 실험 패키징 | `create_rover()` 형태의 생성 함수, 로그 컬럼 정리 | Phase 5로 넘길 로버 API |
| 23 | 지형 위 단일 로버 | RigidTerrain/SCMTerrain 위 주행, 접촉력 관찰 | 단일 환경 테스트 |
| 24 | 개인 로버 정리 | README, 실행법, 결과 예시, 한계점 정리 | 팀 공유용 개인 로버 패키지 |

### Phase 5: 다중 로버 실험 (2학기) - "여러 환경에서 충돌시키고 결과 내기"

| 순서 | 주제 | 배우는 것 |
|------|------|-----------|
| 25 | 공통 환경 API | 평면/장애물/경사/요철 등 여러 실험 환경을 함수로 생성 |
| 26 | 단일 환경 + 로버 4대 | 각자 만든 로버를 같은 환경에 배치하고 충돌/상호작용 확인 |
| 27 | 다양한 환경 + 로버 vs 로버 | 환경 파라미터를 바꾸며 충돌, 이동 거리, 접촉력 비교 |
| 28 | 결과 출력 파이프라인 | CSV(궤적/힘), JSON(설정/메타데이터), 시각 자료를 정리해 AI/온톨로지 레이어로 전달 |
...
│   ├── phase4/                      #   Phase 4: 개인 로버 제작 (각자 이름 폴더)
│   │   └── hojin/                   #     박호진 로버 MVP 코드
│   ├── phase5/                      #   Phase 5: 다중 로버 환경/충돌/결과 출력
│   └── extras/                      #   보너스 예제 (로드맵 외)
│
