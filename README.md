...
| 1학기 Week 11~14 (6월) | Phase 4: 응용 시뮬레이션 | 로버, 드론, 복합 환경 시뮬레이션 | 📋 예정 |
| 2학기 (9~12월) | Phase 5: 자동화 파이프라인 | 파라미터화, 배치 실행, 데이터 출력 | 📋 예정 |
...
> Phase 4에서 이를 로버/드론/환경에 적용합니다.

### Phase 4: 응용 시뮬레이션 (Week 11~14) - "로버 · 드론 · 환경"

| 순서 | 주제 | 배우는 것 | 참고 데모 |
|------|------|-----------|-----------|
| 19 | 로버 시뮬레이션 | Chrono::Robot의 Curiosity/Viper 로버 구동, 궤적 데이터 수집 | `demo_ROBOT_Curiosity_Rigid.py` |
| 20 | 로버 + 지형 상호작용 | SCMTerrain 위 로버 주행, 바퀴-토양 상호작용 | `demo_VEH_HMMWV_DefSoil.py` |
| 21 | 드론 바디 모델링 | ChBody + 조인트 + 모터로 쿼드콥터 구성, 질량/관성 설정 | — |
| 22 | 드론 비행 역학 | Force Functor 추력, 4프로펠러 제어, 호버링/이동 | — |
| 23 | 환경: 외력과 중력 | 바람(외력), 중력 변화(달 1.62/화성 3.72), 환경 파라미터 기록 | — |
| 24 | 통합 시나리오 | 로버 + 드론 + 환경을 결합한 시뮬레이션, 데이터 CSV 출력 | — |

### Phase 5: 자동화 파이프라인 (2학기) - "온톨로지 연동"

| 순서 | 주제 | 배우는 것 |
|------|------|-----------|
| 25 | 파라미터화 시뮬레이션 | 드론/로버/차량 파라미터를 외부 입력(JSON/YAML)으로 수신 |
| 26 | 배치 시뮬레이션 | 여러 시나리오 자동 실행, 결과 수집 |
| 27 | 데이터 출력 파이프라인 | CSV(궤적/힘), JSON(설정), Mesh(형상) 포맷 출력 |
| 28 | 온톨로지 연동 | 지식 레이어에서 파라미터 수신, 결과 RDF 변환 반환 |
