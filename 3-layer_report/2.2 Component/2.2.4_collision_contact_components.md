...
*그림 2.2.4-1. 충돌 형상, 접촉 재질, reporter, logger가 접촉 측정 산출물로 연결되는 흐름*
...
| Collision Backend / Envelope | broadphase/narrowphase backend와 collision tolerance가 접촉 결과에 영향을 줄 때 | collision system/backend, envelope/margin 설정 | backend, envelope, margin, thread/backend 메타데이터 | backend/envelope 메타데이터, visual/collision 비교 |
...
| 선택 항목 | 빠른 prototype | 정밀/확장 선택 | 기록해야 할 메타데이터 |
| --- | --- | --- | --- |
...
*그림 2.2.4-2. 왼쪽은 접촉을 만드는 body, collision shape, material, filter를, 가운데는 Chrono contact pipeline을, 오른쪽은 debug render, CSV, graph, event 산출물을 보여준다. Component 카탈로그에서는 이 흐름을 하나의 API 호출이 아니라 형상, 재질, 실행 조건, reporter, logger의 역할로 나누어 기록한다.*

### Collision/Contact Component 관리 기준표

| Component | 소유 책임 | 의존 대상 | 산출물 | 주의/실패 조건 | 확장 경로 |
| --- | --- | --- | --- | --- | --- |
| Collision Shape | solver가 보는 primitive/compound/mesh, local pose, body role | `ChContactable`, Contact Material, Runtime scale | visual/collision comparison render, shape 메타데이터 | visual mesh와 불일치, 얇은 형상, local pose 누락 | primitive -> compound -> convex/mesh policy |
| Collision Backend / Envelope | collision system type, broad/narrow phase backend, envelope/margin | Runtime Config, Chrono build | backend 메타데이터, collision timing, debug shape render | early contact, tunneling, backend별 contact count 차이 | Bullet -> multicore/parallel backend |
...
| Contact Material | solver contact parameter and material id | Contact method, terrain/tire/chassis component | material effect graph, material 메타데이터 | NSC/SMC mismatch, copied material pointer 공유, unit/provenance 누락 | uniform material -> calibrated pair material |
...
| SMC stiffness/damping 과대 | force peak가 너무 크고 timestep에 민감 | contact material effect graph와 timestep 메타데이터 비교 |
| NSC에서 compliance를 기대 | 침투/변형량 기반 튜닝이 되지 않음 | SMC가 필요한 실험인지 Terrain/Tire 요구사항 재검토 |

### Contact Runtime 관리 기준

Contact 결과는 shape나 material만으로 재현되지 않는다. System class, contact method, solver, timestep, backend, envelope/margin, module availability를 같은 run 메타데이터로 묶어야 한다.

| contact_method | system_class | material_class | solver/runtime | dt_s | collision_backend | envelope_m / margin_m | 모듈 사용 가능성 | 실패 증상 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
...
| FEA/Granular/FSI contact | 모듈별 system/coupling | 모듈별 contact 또는 coupling material | 모듈 solver/coupling step | coupling step 기록 | module backend | domain/contact tolerance | FEA/Granular/FSI 사용 가능성 | 접촉 검증 자료는 모듈 상태 기록 없이는 재현하기 어렵다. |

### Collision Shape Family 카탈로그
...
| Convex/compound | convex hull, multiple primitives | mesh보다 안정적이면서 복잡한 외형 근사 가능 | primitive별 local pose 메타데이터 필요 | 로버 하부 envelope, 복합 장애물 |
| Triangle mesh | triangle mesh collision | 복잡한 terrain/obstacle 형상 표현 | 계산량과 수치 안정성 비용이 큼 | 고정 terrain mesh, 상세 obstacle 검증 |

### 확장 Collision Shape Type 등록 기준
...
| Type group | 공식 enum 값 | 대표 class/API 계열 | 반드시 남길 카탈로그 항목 | 주요 위험 |
| --- | --- | --- | --- | --- |
...
| Unknown/대체 형상 | `UNKNOWN_SHAPE` | import되었거나 지원 범위가 불분명한 형상 | source class string, 생성 방식, 지원되지 않는 동작 | 형상 정책이 실제 solver shape를 재현하지 못함 |

| `collision_shape_manifest.csv/json` column | 의미 |
| --- | --- |
...
| `bounding_box_min/max`, `mesh_path`, `mesh_hash`, `mesh_convexity` | broadphase/debug/asset 재현을 위한 geometry 검증 자료 |
| `thin_shape_risk`, `envelope_m`, `margin_m`, `debug_render` | tunneling, rounded-corner effect, visual/collision mismatch를 추적 |

### Collision Shape API 메타데이터 기준

Chrono collision model은 `ChCollisionShape` 목록과 contact material을 가진다. Component 카탈로그에서는 shape family 이름만 저장하지 말고, solver가 실제로 본 형상의 출처, local frame, mesh policy를 같이 남긴다.

| 형상 사례 | 필요한 메타데이터 | 주의 기준 |
| --- | --- | --- |
...
Collision backend와 envelope/margin은 개별 body가 아니라 Runtime과 collision system 사이의 계약이다. 보고서에서는 body shape만 기록하지 말고, 어떤 collision backend와 margin 조건에서 생성된 접촉인지 메타데이터로 남긴다.

| Component | 소유 책임 | 의존 대상 | 메타데이터 key | 주의/실패 조건 |
| --- | --- | --- | --- | --- |
...
### Collision Backend 검증 기준

`ChSystem::SetCollisionSystemType()`로 선택한 backend는 결과 해석의 일부이다. Bullet과 Multicore는 같은 shape/material을 보더라도 contact count, broad/narrowphase timing, callback 지원 범위가 달라질 수 있으므로 실행 산출물에 backend 정보를 남긴다.
...
| `module_availability` | Multicore/parallel backend가 실제 빌드에서 활성화되었는지 또는 대체 경로인지 구분 |
| `thread_count_collision` | `SetNumThreads` 또는 system thread 메타데이터가 broad/narrowphase timing에 영향을 줄 수 있음 |
...
| `source`, `generation_note` | PNG/CSV가 실제 Chrono 실행 자료인지, 설명용 예시 데이터인지 구분 |
...
| 항목 | 계약 |
| --- | --- |
| family | chassis, wheel, suspension link, terrain, obstacle 같은 collision family 이름을 메타데이터에 둔다. |
...
| 검증 산출물 | debug render에서 included/excluded pair와 reporter pair가 서로 다르게 보일 수 있음을 설명한다. |

#### Collision Family Filter Map 관리 기준

Chrono collision family는 `ChCollisionModel`이 가진 0..15 범위의 family id와 allow/disallow mask로 작동한다. Component 카탈로그에서는 "self-collision을 껐다" 같은 문장 대신 최종 적용된 map을 산출물로 남긴다.

| `collision_family_filter_map.csv/json` field | 필요한 값 |
| --- | --- |
...
| `debug_artifact` | 포함/제외 pair가 표시된 render 또는 pair CSV |
| `failure_mode` | self-collision noise, 필요한 pair 누락, reporter over-filter 같은 위험 |

| 예시 family | 충돌 허용 대상 | 차단 대상 | Reporter 기록 범위 |
| --- | --- | --- | --- |
...
| `terrain` | 시나리오에 따라 chassis/wheel/track/obstacle | terrain-terrain self-pair | terrain-wheel 접촉 검증 |

### Contactable Target 카탈로그

Chrono contact는 두 `ChContactable` 객체 사이의 관계이다. 이 문서의 예시는 `ChBody` 중심이지만, Component 기준에서는 contact target 종류를 명시해야 한다.

| Contact target | Collision 소유 책임 | Reporter 요구사항 |
| --- | --- | --- |
| Rigid `ChBody` | body의 `ChCollisionModel`과 shape/material | `CastToChBody()` 기반 pair reporter 사용 가능 |
| Particle or point contactable | contactable type과 variable DOF 메타데이터 | body name filter가 통하지 않으므로 contactable id/type filter 필요 |
| FEA node/segment/triangle | `ChMesh` contact surface와 selected nodes/faces | body-only reporter는 미지원 target으로 기록 |
...
### Contact Container와 Reporter 구분

Contact Container는 Chrono가 한 step의 contact를 담는 runtime 객체이고, Reporter는 그 container를 어떤 schema로 읽을지 정하는 Component이다. `GetContactContainer().ReportAllContacts(callback)` 호출은 단순 문법이 아니라 아래 기준을 실행하는 지점이다.

| Runtime 항목 | Chrono 역할 | Component 검증에 필요한 정보 |
| --- | --- | --- |
| `GetContactContainer()` | system이 가진 contact container 접근 | container class/type, contact method, backend를 run 메타데이터에 기록 |
...
| Multicore container | parallel collision/contact backend의 container | callback force/torque 지원 범위와 생성 방식을 명시 |

코드 블록은 구현 기준을 보여주는 예시이다. 이 2.2 문서는 문법 설명서가 아니라 Component의 소유 책임, 메타데이터, 검증 자료, 실패 조건, 확장 경로를 고정하는 카탈로그이고, API 호출 순서 자체는 2.1 Command 단계에서 다룬다.

### Contact Callback Family 카탈로그

Chrono의 callback은 모두 같은 "reporter"가 아니다. Broadphase/narrowphase callback은 contact 생성 전 collision pipeline에 개입하고, add-contact callback은 container에 contact가 들어갈 때 pair composite material을 바꿀 수 있으며, report callback은 이미 추가된 contact를 읽어 CSV/graph로 후처리한다. Debug visualization callback은 shape/contact 표시용 검증 hook이다.

| Callback family | 작동 단계 | Chrono hook | 입력 객체 | contact/material 수정 가능 여부 | 카탈로그 산출물 | 미지원/대체 처리 방식 |
| --- | --- | --- | --- | --- | --- | --- |
| Broadphase callback | broadphase 이후 가까운 pair | `ChCollisionSystem::RegisterBroadphaseCallback` / `OnBroadphase` | 두 `ChCollisionModel` pointer | 반환값으로 narrowphase pair를 건너뛸 수 있음 | `contact_callback_manifest.csv/json`, family filter map | backend/PyChrono binding 미지원이면 family/mask와 reporter filter로 대체 |
| Narrowphase callback | narrowphase 이후 겹치는 pair | `RegisterNarrowphaseCallback` / `OnNarrowphase` | `ChCollisionInfo` | 생성된 collision info를 contact로 보낼지 결정할 수 있음. geometry override는 반드시 문서화 | callback manifest, modified contactinfo log | mutation 미지원이면 report-only 검증 자료로 낮춰 기록 |
| Add-contact callback | contact point가 container에 추가되는 시점 | `ChContactContainer::RegisterAddContactCallback` / `OnAddContact` | `ChCollisionInfo`, `ChContactMaterialComposite` | pair별 composite material 수정 가능 | `contact_pair_material_policy.csv/json` | material mutation disabled이면 original/composite material ids만 기록 |
| Report-contact callback | contact가 container에 이미 들어간 뒤 | `ReportAllContacts(callback)` / `OnReportContact` | pA, pB, contact frame, force/torque, contactables | 읽기/후처리 전용. `false` 반환 시 scanning 중단 | contact probe CSV, force/torque graph, reporter scope render | non-body contactable cast 실패를 `unsupported_contactable_type`으로 남김 |
| Visualization callback | collision debug render | `RegisterVisualizationCallback` / `Visualize(flags)` | collision shape/contact visualization request | 물리 상태는 바꾸지 않고 debug drawing만 수행 | `contact_frame_debug_manifest.csv/json`, render PNG | 렌더링 backend가 지원되지 않으면 대체 도식과 사유를 함께 기록 |

| `contact_callback_manifest.csv/json` field | 의미 |
| --- | --- |
| `callback_id`, `callback_family`, `class_name` | user callback과 Chrono hook family를 안정적으로 구분 |
| `registered_on`, `phase`, `chrono_hook` | collision system인지 contact container인지, 어느 phase에서 호출되는지 |
| `input_contract` | `ChCollisionModel`, `ChCollisionInfo`, `ChContactMaterialComposite`, `ReportContactCallback`가 받는 인자 |
| `mutation_policy` | pair skip, geometry override, composite material edit, report-only, visualize-only 중 하나 |
| `filter_rule`, `pair_scope`, `contactable_type_policy` | body pair, family/mask, non-body target 처리 |
| `artifact`, `evidence_level`, 대체 사유 | CSV/render/manifest가 실제 실행 자료인지 대체 예시인지 구분 |

### Contact Reporter Component 관리 기준

Reporter는 "반복문에서 callback을 호출하는 코드"가 아니라 접촉 데이터를 어떤 범위와 좌표계로 집계할지 정하는 Component이다. 아래 기준을 먼저 정해두면 2.1 Command 단계의 `ReportAllContacts` 호출 설명과 혼동되지 않는다.

| 계약 항목 | Component가 소유하는 결정 | 로그/메타데이터 컬럼 |
| --- | --- | --- |
...
*그림 2.2.4-3. 접촉 측정 장면을 설명하는 deterministic schematic이다. 파란 body와 빨간 obstacle은 각각 collision shape을 가진 contactable이고, 하늘색 화살표는 approach velocity, 초록 화살표는 contact normal, 주황 화살표는 reporter가 기록할 contact force 방향을 나타낸다.*

| 시각 자료 |
| --- |
| ![Visual collision difference](images/renders/collision_visual_vs_collision_shape.png) |

*그림 2.2.4-4. 왼쪽은 사람이 보는 visual shape, 오른쪽은 물리 계산에 쓰는 단순 collision primitive를 분리해 보여준다. 시각화용 외형과 물리 계산용 형상은 서로 다를 수 있다.*
...
| --- | --- | --- |
| Common `ChContactMaterial` | static/sliding friction, rolling/spinning friction, restitution | sliding/rolling/spinning resistance와 bounce 메타데이터 |
| `ChContactMaterialNSC` | cohesion, compliance, complianceT, compliance rolling/spinning, dampingF | hard-contact solver 안의 compliance/damping 메타데이터; zero compliance는 rigid contact assumption |
...
| `source`, `calibration_note` | 문헌값, 실험값, 대체 예시값 구분 | 대체 그래프를 실제 Chrono material calibration으로 과대 해석하지 않는다. |
| `compatible_system` | `ChSystemNSC`, `ChSystemSMC`, Vehicle/terrain coupling 조건 | Runtime 메타데이터와 같은 값이어야 한다. |

### Contact Pair Material Policy 관리 기준

표면 material 카탈로그는 각 shape가 가진 material을 설명하고, pair material policy는 두 material이 만났을 때 composite property를 그대로 쓸지 callback으로 바꿀지를 설명한다. 이 둘을 분리해야 terrain patch friction, tire model friction, solver contact material이 섞이지 않는다.

| `contact_pair_material_policy.csv/json` 항목 | 필요한 이유 |
| --- | --- |
| `material_a_id`, `material_b_id`, `body_or_shape_a`, `body_or_shape_b` | pair별 override가 어느 표면 조합에 적용되는지 고정 |
...
| `calibration_source` | 실험값, 문헌값, 해석식 기반 대체 예시값 구분 |
| `evidence_graph`, `debug_artifact` | material effect graph와 contact pair render 연결 |
...
| 대체 예시 graph | material trend를 보여주는 deterministic graph이며 실제 material calibration 검증은 아님 |
...
*그림 2.2.4-5. 같은 충돌이라도 접촉 재질이 더 단단하면 짧고 큰 force peak가 나타나고, 더 부드럽거나 damping이 큰 조건은 peak가 낮고 지속 시간이 길어진다. Contact Material은 "닿았는가"가 아니라 "닿은 뒤 어떤 힘으로 반응하는가"를 바꾸는 Component이다.*
...
| Reporter 계열 | 소유 책임 | 예상 schema | 쓰지 말아야 할 경우 |
| --- | --- | --- | --- |
...
*그림 2.2.4-6. 노란 점은 모두 실제 접촉 후보이지만, 초록 bracket이 감싼 rover-obstacle pair만 이 reporter의 CSV로 들어간다. wheel-ground contact는 같은 step에 존재해도 `body_a`/`body_b` filter 밖이면 다른 logger나 terrain probe에서 다뤄야 한다.*
...
예시 코드가 생성하는 `outputs/csv/collision_contact_probe.csv`는 `contact.force_torque` component id로 force/torque, frame, pair-order, `ComputeContactForces_called` 필드를 함께 포함한다. 별도 `contact_force_torque_probe.csv`는 body/contactable total과 pair reporter sum을 비교하는 실제 실행 확장 산출물로 남겨둔다.
...
Chrono에서 contact force는 여러 경로로 읽을 수 있다. `ReportContactCallback`의 force/torque는 contact frame 기준 pair contact 검증 자료이고, `ChBody::GetContactForce()`/`GetContactTorque()`나 contact container의 `ComputeContactForces()` 이후 `GetContactableForce/Torque()`는 body/contactable별 resultant 검증 자료이다. 카탈로그는 어느 값이 대표 force인지 명시해야 한다.

| Force/Torque 출처 | 범위 | 필요한 메타데이터 | 섞으면 생기는 문제 |
| --- | --- | --- | --- |
| Pair reporter force | 필터링된 contact point 또는 pair | `body_a`, `body_b`, `pair_order`, `plane_coord`, `force_on`, world/contact frame | 특정 pair force를 body 전체 force처럼 해석 |
| Pair reporter torque | 계산된 rolling/spinning torque | `react_torques`, `torque_frame`, rolling/spinning material fields | 미지원 zero와 실제 측정 zero 혼동 |
| Body total force/torque | 특정 `ChBody`에 작용한 모든 contact | body id, `GetContactForce`, `GetContactTorque`, aggregation step | terrain/wheel/obstacle contact가 모두 섞임 |
| Contactable total force/torque | 일반 `ChContactable` resultant | `ComputeContactForces_called`, contactable id/type, container class | non-body target을 누락하거나 stale cache 사용 |
| Tire/terrain subsystem force | vehicle/terrain 계열 출력 | tire id, terrain id, sync/advance order | solver body contact force와 tire model force 혼동 |
...
| Callback field | 본 보고서의 관리 기준 | 필수 점검 |
| --- | --- | --- |
| `pA`, `pB` | 두 contactable의 절대 contact point | 좌표 단위는 meter이며 `body_a/body_b` 또는 contactable id와 연결 |
| `plane_coord` | contact frame. X column이 contact normal | CSV/debug render의 normal vector가 이 basis와 일치해야 함 |
| `distance` | 부호가 있는 separation/penetration distance | penetration 크기를 쓸 때는 `penetration_m = max(0, -distance)`로 저장 |
| `eff_radius` | contact 지점의 유효 곡률 반지름 | mesh/rounded/capsule contact 진단에 사용 |
| `react_forces` | contact frame 기준 reaction force | world-frame 성분을 저장하려면 frame transform과 `frame=world` 메타데이터 필요 |
| `react_torques` | 계산된 rolling/spinning torque | 미지원 zero와 실제 측정 zero를 구분 |
| `contactobjA/contactobjB` | 일반 `ChContactable` pointer | body-only cast를 쓰면 non-body contactable 미지원 사실을 명시 |
| pair sign | 힘 방향은 pair 순서와 normal 방향에 따라 달라짐 | debug vector와 body total contact force로 합산 부호 검증 |
...
| coordinate frame | CSV의 `contact_fx_N`, `contact_fy_N`, `contact_fz_N`은 world frame 기준이다. contact local frame이면 컬럼명 또는 메타데이터에 `local`을 명시한다. |
...
*그림 2.2.4-7. 충돌 순간 접촉력 peak가 발생한다. peak 크기와 지속 시간은 충돌 강도와 timestep 안정성 평가에 사용된다.*

| 그래프 |
| --- |
| ![Contact count graph](images/graphs/collision_contact_count_graph.png) |

*그림 2.2.4-8. 접촉 개수는 충돌이 몇 개의 contact point/pair로 해석되는지 보여준다. 급격히 튀는 값은 timestep, collision shape, solver 설정을 점검해야 한다는 신호일 수 있다.*

| 그래프 |
| --- |
| ![Contact force components graph](images/graphs/collision_contact_force_components_graph.png) |

*그림 2.2.4-9. 접촉력 성분을 분리해 보여준다. 진행 방향 반대 성분과 수직 성분을 함께 보면 force vector가 렌더의 contact normal과 맞는지 검증할 수 있다.*
...
*그림 2.2.4-10. `first_contact` 시점을 타임라인으로 보여준다. System 실험에서는 이 event를 성공/실패 판정, 충돌 전후 상태 비교, 자동 리포트 생성에 연결할 수 있다.*
...
*그림 2.2.4-11. 노란 점은 contact point, 초록/주황 marker는 contact normal과 force vector 개념을 나타낸다. Contact Force Logger의 `contact_fx_N`, `contact_fz_N` 성분과 함께 보면 접촉 방향이 물리적으로 일관적인지 확인할 수 있다.*

### 주의/실패 조건 카탈로그

| 주의/실패 조건 | 증상 | 주로 점검할 Component | 확인 자료 | 보완 방법 |
| --- | --- | --- | --- | --- |
...
| SMC stiffness/dt mismatch | force가 진동하거나 폭주 | Contact Material, Runtime | material effect graph, dt 메타데이터 | stiffness/damping 재조정, timestep 축소 |
...
| envelope causing early contact | 시각적으로 닿기 전에 contact count 발생 | Collision Backend / Envelope | envelope 메타데이터, debug collision render | envelope/margin 조정 |
...
| method/material mismatch | parameter가 기대대로 작동하지 않음 | Runtime/Material | system class vs material class 메타데이터 | NSC/SMC system-material 일치 |
...
| --- | --- | --- |
| `2.2.2` Rover/Vehicle | chassis/wheel/tire/track shoe body id, collision shape policy, tire/contact family | body-pair contact force, wheel/obstacle event, shape failure 검증 자료 |
| `2.2.3` Terrain | ground/obstacle/rigid patch/SCM component id, `terrain_mu_query`, `contact_material_id` | solver contact force, `contact_material_mu`, terrain-wheel pair 검증 자료 |
| `2.2.5` Data/Visualization | CSV schema, 메타데이터 logger, graph/render output policy | contact probe CSV, event timeline, debug render links |
| `2.2.1` Runtime/Config | contact method, solver, dt, backend, module availability | runtime-sensitive 실패 유형s and reproducibility 메타데이터 |
...
| `outputs/csv/collision_backend_comparison.csv` / `outputs/json/collision_backend_comparison.json` | 예시 데이터와 실제 실행 데이터 모두에서 backend/contact-method를 비교하기 위한 행 |
...
| `outputs/json/collision_model_registry.json` | contactable-to-collision-model registry and shape/material 소유 책임 |
...
| `outputs/csv/contact_callback_manifest.csv` / `outputs/json/contact_callback_manifest.json` | report/debug callback family 카탈로그 |
