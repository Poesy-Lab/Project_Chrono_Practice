...
**설명.** 왼쪽은 접촉을 만드는 body, collision shape, material, filter를, 가운데는 Chrono contact pipeline을, 오른쪽은 debug render, CSV, graph, event 산출물을 보여준다. Component 카탈로그에서는 이 흐름을 하나의 API 호출이 아니라 형상, 재질, 실행 조건, reporter, logger의 역할로 나누어 기록한다.
