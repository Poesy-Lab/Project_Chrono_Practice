...
*그림 2.2.3-1. 환경/Terrain Component가 지형 형상, 접촉 재질, 지형 조회, 주행 검증 자료로 이어지는 관계*
...
*그림 2.2.3-2. 같은 full-scene 이미지에서 회색 평판이 Ground Component이다. 가장 단순한 환경이지만 모든 충돌/주행 실험의 기준 높이와 기준 마찰을 제공한다.*

| 시각/그래프 자료 |
| --- |
| ![Terrain contact force probe](images/graphs/terrain_contact_force_probe.png) |

*그림 2.2.3-3. body가 ground와 접촉할 때 발생하는 접촉력 변화를 기록한 probe 그래프이다. 접촉력 peak가 너무 크면 초기 침투나 timestep 문제를 의심한다.*
...
*그림 2.2.3-4. 같은 full-scene 이미지에서 빨간 박스와 갈색 원통이 Obstacle Component이다. 고정 장애물은 충돌 기준점이 되고, 동적 장애물은 별도 질량과 관성을 가진 body로 다룬다.*
...
*그림 2.2.3-5. 회색/주황색 patch는 서로 다른 contact material 영역을 나타낸다. 같은 wheel path라도 영역별 마찰이 달라지면 구동력, slip, 제동거리 해석이 달라진다.*

| 시각/그래프 자료 |
| --- |
| ![Contact material friction map](images/graphs/terrain_contact_material_friction_map.png) |

*그림 2.2.3-6. 위치별 마찰 계수 map 예시이다. 같은 로버라도 contact material이 달라지면 미끄러짐, 제동거리, 접촉력 peak가 달라진다.*
...
*그림 2.2.3-7. 파란 반투명 영역은 `RigidTerrain` patch의 공간 범위, 주황 선은 wheel path, 빨간 박스는 같은 patch 위에 놓인 obstacle 예시이다. Ground body와 달리 Vehicle module에서는 patch 단위로 재질, texture, heightmap/mesh 형상을 묶어 terrain interface를 구성한다.*
...
*그림 2.2.3-8. VSG로 캡처한 Heightmap 형태의 rigid patch 개념이다. RigidTerrain은 바퀴 하중에 의해 변형되지 않고, 지형 높이/법선/마찰 정보를 차량 subsystem에 제공한다.*

| 시각/그래프 자료 |
| --- |
| ![Terrain height and sinkage profile](images/graphs/terrain_height_sinkage_profile.png) |

*그림 2.2.3-9. 이 그래프에서는 높이 profile을 지형 형상 입력/reference로 읽는다. Heightmap은 형상 입력이고, sinkage는 변형 지형에서 나타나는 결과로 구분해야 한다.*
...
*그림 2.2.3-10. 붉은 marker는 wheel/track path이고, 표면의 낮아진 셀은 SCM 변형 지형에서 기대하는 침하/바퀴 자국 개념이다.*

| 시각/그래프 자료 |
| --- |
| ![Terrain height and sinkage profile](images/graphs/terrain_height_sinkage_profile.png) |

*그림 2.2.3-11. 이 그래프에서는 sinkage profile을 SCM/변형 지형의 출력으로 읽는다. 실제 SCM에서는 soil parameter와 wheel load에 따라 침하량이 결정된다.*
...
*그림 2.2.3-12. 기울어진 patch는 slope frame, 빨간 화살표는 gravity, 주황 화살표는 wind/외력, 초록 화살표는 진행 방향 조건을 나타낸다. Environment Field는 형상이 아니라 실험 조건 Component로 기록해야 한다.*
...
*그림 2.2.3-13. 왼쪽은 지형 선택지, 가운데는 공식 모듈 조건, 오른쪽은 실제 실행 검증 자료와 대체 표기 기준이다. DEM/Granular, FEA, CRM 계열은 모듈 사용 가능성과 실제 실행 산출물이 없으면 "공식 검증"이 아니라 개념 설명, 기능 색인, 스키마 예시 상태로만 표기한다.*
