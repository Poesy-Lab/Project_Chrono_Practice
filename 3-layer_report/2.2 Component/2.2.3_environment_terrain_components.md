...
| FlatTerrain | Vehicle tire/controller smoke test에 infinite flat query plane이 필요할 때 | `veh.FlatTerrain`, `ChTerrain` query interface | height, friction, query frame, no-collision flag | flat terrain metadata |
...
| RandomSurfaceTerrain | 반복 가능한 rough surface sweep가 필요할 때 | random surface terrain family | seed, roughness, patch size, generated statistics | roughness metadata, height profile |
...
### 2.1 Command catalog와의 경계

Terrain 문서에는 API 이름이 많이 나오지만, 이 절의 목적은 호출 순서 암기가 아니라 Component가 소유하는 지형 조건, 접촉 권한, probe 출력, fallback 정책을 고정하는 것이다.

| API call | 2.1 command 관심사 | 2.2 terrain component 관심사 | 필요한 산출물/probe |
|---|---|---|---|
...
| `veh.SCMTerrain(system)` | class import와 초기화 | soil parameter, grid spacing, wheel/track load coupling | sinkage/rut profile, soil metadata |
...
Vehicle 계열 지형은 공통적으로 vehicle이 질의할 수 있는 terrain interface를 제공한다. 개발자 관점에서는 "평평한지", "형상이 파일에서 오는지", "바퀴 하중으로 변형되는지", "입자/연속체 해석이 필요한지"를 먼저 나누면 선택이 빠르다.
...
| Terrain | Tire/Track 조합 | Contact source | Module dependency | 주의할 점 |
|---|---|---|---|---|
...
| `RigidTerrain` patch | `RigidTire`, `TMeasy`, `Fiala`, simple track shoe | terrain patch material + tire model/contact | Vehicle | patch material, tire model, contact method(NSC/SMC)를 metadata에 같이 저장한다. |
...
| FEA terrain/tire | deformable tire/terrain 구조 해석 | finite element contact/coupling | FEA | mesh/material law와 solver 설정을 Runtime metadata로 고정해야 한다. |
...
### Terrain Component Contract Matrix
...
| RigidTerrain Patch | patch pose, extents, material, texture, mesh/heightmap file path | `ChSystem`, `ChTerrain` interface, Runtime | rigid patch render, height/friction metadata | `Initialize()` 누락, origin/scale 오류, patch boundary 미기록 | flat patch -> mesh/heightmap patch -> CRG road |
| Heightmap/Mesh Patch | height source file, scale, origin, interpolation assumption | RigidTerrain, CSV/Metadata Logger | height profile graph, terrain render | height scale 부호 오류, file path drift, z offset 누락 | synthetic height -> measured heightmap/mesh |
...
| Environment Field | gravity vector, slope frame, wind/external force convention | Runtime, Chassis, Metadata Logger | environment field render, run metadata | 중력 방향/단위 오류, 외력 frame 미기록 | constant gravity -> slope/wind profile |
| Advanced Terrain | DEM/FEA/CRM domain, particle/mesh spacing, active domain, coupling rule | Chrono modules, Runtime, solver/timestep policy | domain metadata, performance/stability log | 계산 비용 과대, module availability 누락, coupling timestep 불일치 | SCM/RigidTerrain -> DEM/FEA/CRM |
...
| `GetPoint(loc)` | surface 위의 3D 점 | m, world frame | tire contact, render/debug | mesh/CRG terrain은 좌표 원점과 진행 방향을 metadata에 남긴다. |
...
공식 terrain 문서 기준으로 height/normal/friction query는 모든 terrain 결과를 검증하는 만능 증거가 아니다. 이 query는 주로 semi-empirical tire model이 terrain surface를 읽을 때 중요하고, SCM/Granular/FEA terrain처럼 Chrono collision/contact 또는 coupling으로 상호작용하는 변형 지형에서는 별도 contact/deformation evidence가 필요하다.

| Terrain family | Query evidence로 충분한가 | 추가로 필요한 evidence |
|---|---|---|
| `FlatTerrain` | height/friction query smoke test에는 충분 | collision/event 검증에는 부적합. FlatTerrain은 collision/contact geometry를 소유하지 않는다는 metadata가 필요 |
...
| `SCMTerrain` | initial surface query나 plot metadata에는 유효 | soil parameter, grid spacing, sinkage/rut/node evidence, moving patch, contact/coupling source |
...
Query probe는 어느 consumer가 그 값을 쓰는지와 query placeholder 여부를 함께 기록해야 한다. 특히 CRM 문서의 placeholder-style query처럼 값이 terrain force/coupling validation을 의미하지 않는 경우가 있으므로, query가 유효한 범위를 metadata로 제한한다.

| Query metadata field | Required content |
|---|---|
...
| `placeholder_policy` | CRM/extension/fallback에서 query가 approximate 또는 not-valid-for-force인지 |
...
`AddPatch` 호출은 Command 단계의 syntax이고, 2.2 Component catalog에서는 patch가 어떤 source, frame, material, tiling, collision authority를 소유하는지 manifest로 남긴다.

| Patch manifest field | Required content |
|---|---|
...
고급 지형은 "더 복잡한 surface"가 아니라 query authority, contact authority, tire/track 호환성, module availability, timestep 비용이 함께 바뀌는 Component 선택이다. 따라서 factory는 class 이름만 고르는 함수가 아니라 metadata와 fallback까지 같이 내보내는 catalog entry여야 한다.

| 시각 자료 |
|---|
| ![Advanced terrain component selection catalog](images/renders/terrain_advanced_selection_catalog.png) |

**설명.** 왼쪽은 terrain choice, 가운데는 공식 module gate, 오른쪽은 live evidence와 fallback label이다. DEM/Granular, FEA, CRM 계열은 module availability와 live artifact가 없으면 "공식 validation"이 아니라 concept/indexed/schema 상태로만 표기한다.

### Official Advanced Terrain Grounding

Chrono Vehicle terrain 계열은 `chrono::vehicle::ChTerrain`을 기준으로 height, normal, friction query와 `Synchronize`/`Advance` lifecycle을 맞춘다. 고급 지형은 여기서 끝나지 않고, 지형 내부가 particle domain, FEA mesh, SPH-FSI coupling 중 무엇을 소유하는지까지 catalog identity에 포함해야 한다.

| Official terrain | Chrono grounding | Module/build gate | Minimum live evidence | Catalog claim |
|---|---|---|---|---|
| `chrono::vehicle::GranularTerrain` | rectangular spherical-particle bed, DEM contact, optional moving patch | Vehicle terrain + contact backend, with `multicore`/DEM/legacy `granular_gpu`/custom co-sim identity recorded | particle radius/density/count, domain size, contact material, contact method, timestep, moving-patch state, source tag | particle-domain/contact evidence only; no continuum soil claim |
| `chrono::vehicle::FEATerrain` | FEA deformable terrain backed by `chrono::fea` mesh/elements and Drucker-Prager style soil parameters | Vehicle + FEA support, mesh and element implementation available | mesh id, element type/count, material law, boundary conditions, solver dt, deformation/stress/strain artifact | FEA terrain validation only after mesh state and material response artifact exist |
| `chrono::vehicle::CRMTerrain` | SPH-based continuum representation model through Chrono::FSI-SPH | Vehicle + FSI-SPH support, SPH/BCE files or constructed SPH domain available | SPH spacing, BCE/marker files or generated domain, active domain, CFD/MBD step sizes, coupled advance order | CRM validation only from SPH/FSI coupling outputs |

### Advanced Terrain Selection Catalog

| Terrain Component | Owns | Best use | Compatible tire/track/contact | Required metadata | Failure modes |
|---|---|---|---|---|---|
...
| `GranularTerrain`/DEM terrain | particle domain, spherical particle material, DEM contact state | particle-scale sand/soil interaction | rigid/FEA tire, track shoe, contact/coupling layer | particle count/radius/density, domain size, contact method, backend identity, settling/checkpoint hash, dt | particle 수 과대, timestep instability, backend/module dependency 누락, fallback을 validation으로 오해 |
| `FEATerrain` | `chrono::fea` mesh, ANCF/solid element type, constitutive law | local structural deformation, deformable road/soil block | FEA tire, rigid body contact, mesh contact | mesh id/path, element type/count, material law, boundary condition id, solver settings, stress/strain artifact | mesh 품질 문제, boundary condition 누락, stress/strain log 없음, material parameter drift |
...
### DEM / Granular Backend Identity Card

DEM 계열은 `GranularTerrain` 하나로만 끝나지 않는다. 같은 "granular" label이라도 official Vehicle terrain, external DEM engine, OpenMP co-sim, SPH-style extension은 evidence와 실패 모드가 다르므로 backend identity를 먼저 박아야 한다.

| Backend identity | 허용 의미 | Required metadata | Claim rule |
|---|---|---|---|
| `vehicle_granular` | `chrono::vehicle::GranularTerrain` live run | `chrono_class`, particle radius/density/count, length/width/layers, contact material id, contact method, moving-patch flag, `solver_dt_s`, source tag | `module_availability=true`와 particle/contact output이 있을 때만 module-backed evidence |
| `dem_gpu` | DEM-Engine 또는 legacy GPU granular 경로로 관리되는 확장 | backend name/version, GPU/device, particle radius/density/count, domain size, kernel/contact model, checkpoint hash, timestep | local live artifact 없으면 indexed extension으로만 표기 |
...
| `granular_cosim_sph` | SPH/continuum coupling으로 granular-like response를 근사하는 확장 | SPH spacing, coupling body ids, rheology, active domain, timestep, output directory hash | DEM validation이라고 쓰지 말고 CRM/SPH-derived evidence로 표기 |

### FEA Terrain Minimum Evidence Contract

`FEATerrain`은 mesh card만 있다고 검증된 지형이 아니다. 아래 필드가 빠지면 FEA terrain catalog에는 "schema/indexed"만 붙이고, stress/strain/deformation artifact가 생긴 뒤에야 live evidence로 올린다.
...
| Runtime artifact | `deformation_or_stress_artifact`, stress/strain output path or rendered deformation snapshot |

### CRM Evidence Guard

CRM은 `GetHeight`/`GetCoefficientFriction` query가 아니라 SPH-FSI coupling output으로 판단해야 한다. 따라서 일반 height/friction probe만 있는 CSV는 `CRMTerrain` validation이 아니라 terrain interface smoke evidence로만 취급한다.

| Evidence source | 허용 claim | 금지 claim |
|---|---|---|
| `CRMTerrain` live SPH/FSI output with SPH particles, BCE markers, active domain, coupled step | CRM module-backed evidence | 단순 rigid/flat terrain과 동일한 friction terrain으로 해석 |
| CRM API/index entry without local run artifact | indexed official capability | local validation, force/sinkage fidelity claim |
| fallback analytic height/friction probe | schema smoke test 또는 plotting fixture | CRM soil flow, SPH continuum validation |
| concept render | ownership/gate 설명 | runtime correctness 또는 module availability 증거 |

### Query Authority vs Contact Authority

| Component | Query authority | Contact authority | Component 해석 |
|---|---|---|---|
...
| `CRGTerrain` | road s/l frame 기반 profile query | tire model 또는 별도 road contact 설정 | OpenCRG profile의 좌표계와 version을 component evidence로 남긴다. |
| `SCMTerrain` | deformable grid state | soil grid와 wheel/track load coupling | sinkage와 rut가 terrain 출력이다. |
| `GranularTerrain`/DEM | particle field and contact state | particle contact/coupling | domain, particle material, backend identity, timestep이 Component 계약의 일부다. |
...
| 후보 | Collision geometry 생성 | `ChTerrain` query 제공 | Typical tire/contact | 쓰지 말아야 할 경우 | Metadata fields |
|---|---|---|---|---|---|
...
| Tire/Track family | `FlatTerrain` | `RigidTerrain` | `CRG` | `SCM` | DEM | FEA | CRM/FSI | Required evidence |
|---|---|---|---|---|---|---|---|---|
...
### Terrain Metadata Contract

| Key | 의미 |
|---|---|
| `terrain_component_id` | run 안에서 지형을 식별하는 안정 ID |
| `terrain_type` | `core_ground`, `flat`, `rigid_patch`, `crg`, `scm`, `dem`, `fea`, `crm` 같은 catalog type |
| `chrono_class` | 실제 Chrono/PyChrono class 또는 fallback helper 이름 |
| `module_dependency` | Vehicle, FEA, FSI/CRM, Granular, Multicore 등 필요한 module |
| `module_availability` | 현재 환경에서 import/build 가능한지와 fallback 여부 |
...
| `fallback_used` | PyChrono/module 미사용 시 analytic/render fallback 사용 여부 |

### Terrain Manifest Split

System 단계에서 terrain을 재현하려면 하나의 `terrain_type`만으로는 부족하다. rigid patch, material region, deformable domain, query probe를 서로 다른 manifest로 나눠야 fallback schema와 live module evidence를 섞지 않는다.

| Manifest | Owned by | Required fields |
|---|---|---|
| `terrain_component_manifest.json` | `terrain.interface` | `terrain_component_id`, `terrain_type`, `chrono_class`, `module_dependency`, `module_availability`, `query_authority`, `contact_authority`, `sync_order`, `advance_order`, `fallback_policy` |
...
현재 fallback generator는 `terrain_component_manifest.json`, `terrain_patch_manifest.csv/json`, `terrain_material_region_map.csv/json`, `terrain_deformable_domain_manifest.json`, `scm_soil_profile_manifest.csv/json`, canonical `terrain_surface_probe.csv`를 생성한다. `environment_terrain_height_friction_sinkage.csv`는 같은 surface probe schema를 그래프 설명용 이름으로 유지한 compatibility 파일이다.

### SCM Node / Moving Patch Evidence Contract

`SCMTerrain`은 deformable height-field model이며, grid 전체를 항상 materialized object로 보지 않는다. 따라서 SCM evidence는 surface picture만이 아니라 touched/deformed node와 moving patch policy를 기록해야 한다.

| SCM evidence field | Required content |
|---|---|
...
| `moving_patch` | body id, OOBB center/dims, active patch count, no-patch fallback |
...
| `sinkage_artifact` | profile CSV/graph, rut render, or live node export path/hash |

### Terrain Lifecycle / Advance Contract

Terrain Component는 object 생성 후 자동으로 의미가 생기는 것이 아니라, System loop에서 누가 synchronize/advance를 소유하는지에 따라 evidence 해석이 달라진다.

| Terrain family | `synchronize_owner` | `advance_owner` | Vehicle advance relationship | Contact force owner | `dt_field` | Module gate | Fallback policy |
|---|---|---|---|---|---|---|---|
| `FlatTerrain` | terrain wrapper or scenario loop | normally no internal state | `vehicle.Synchronize(time, inputs, terrain)` reads query values | tire model/controller, not terrain collision | `vehicle_dt_s` | Vehicle | schema-only allowed for query smoke |
...
| `SCMTerrain` | SCM terrain wrapper | SCM grid state advances after contact/load update | terrain and vehicle must share step/order; moving patch follows body | SCM soil-wheel/track coupling | `solver_dt_s`, `terrain_dt_s`, grid spacing | Vehicle SCM | fallback graph is sinkage schema only |
...
| Evidence level | 의미 | 이 폴더에서의 예 |
|---|---|---|
| Concept render | Component ownership과 경계 설명용 그림 | `terrain_advanced_selection_catalog.png`, `terrain_scm_deformation_component.png` |
| Fallback analytic probe | PyChrono 없이 생성한 예시 CSV/그래프 | `environment_terrain_height_friction_sinkage.csv`, `terrain_height_sinkage_profile.png` |
| PyChrono live probe | 실제 Chrono terrain/tire run에서 나온 height/contact probe | 향후 실행 로그와 CSV에 `source=pychrono_live`로 기록 |
| Chrono module validation | DEM/FEA/CRM module까지 포함한 검증 run | module availability, build hash, timestep, active domain을 metadata에 포함 |

| Evidence gate | 통과 조건 | 금지되는 overclaim |
|---|---|---|
| Official module-backed validation | `module_availability=true`, official class identity, live artifact path/hash, required schema fields가 모두 존재 | fallback analytic plot을 module validation으로 표기 |
| Indexed official capability | official API/reference와 catalog metadata는 있으나 local run artifact 없음 | force, sinkage, deformation, soil-flow 정확도 claim |
| Fallback analytic/schema | PyChrono/module 없이 생성한 deterministic fixture 또는 schema example | `GranularTerrain`/`FEATerrain`/`CRMTerrain` live evidence claim |
| Concept render | component boundary, gate, ownership 설명용 그림 | runtime correctness, module availability, solver stability claim |

고급 지형은 module과 빌드 옵션 의존성이 커서 동일한 코드 경로로 무리하게 실행하기보다, terrain factory처럼 선택지를 분리해 두는 편이 관리하기 쉽다. factory 반환값은 terrain 객체뿐 아니라 catalog metadata를 함께 내보내야 한다.
...
| 지형 좌표계 | `world_frame`, ISO `vehicle_frame`, `render_camera_up`을 분리해 확인. 이 보고서의 차량/지형 schema는 X-forward/Y-left/Z-up을 사용하고, 렌더 camera up은 시각화 backend metadata로 별도 기록한다. |
