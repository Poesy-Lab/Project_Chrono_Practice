...
![Rigid Body 구성 요소](images/mermaid/command_rigid_body_composition.png)

*그림 2.1.7-1. `ChBody` 또는 `ChBodyEasy*`가 질량, 관성, 위치, 시각 형상, 충돌 형상, 상태 출력으로 구성되는 구조*
...
## 2.1.7.1 ChBody 직접 구성 예시

`ChBodyEasy*` 계열은 빠른 실험에 유용하지만, Component 단계에서는 질량, 관성, visual shape, collision shape를 분리해서 설명해야 하는 경우가 많다. 이때는 `ChBody`를 직접 생성하고 필요한 속성을 차례로 추가한다.

```python
body = chrono.ChBody()
body.SetName("custom_chassis")
body.SetMass(12.0)
body.SetInertiaXX(chrono.ChVector3d(0.8, 1.2, 1.5))
body.SetPos(chrono.ChVector3d(0, 0.5, 0))
body.SetRot(chrono.QUNIT)
system.Add(body)
```

## 2.1.7.2 Component 및 System 연결

| Rigid Body Command | Component 단계 의미 | System 단계 사용 |
| --- | --- | --- |
| `ChBody` | 질량/관성/형상을 직접 정의하는 강체 | custom chassis, payload |
| `ChBodyEasyBox` | 상자형 강체 빠른 생성 | ground, wall, chassis prototype |
| `ChBodyEasyCylinder` | 원통형 강체 빠른 생성 | wheel prototype |
| `SetFixed(True)` | 움직이지 않는 기준 물체 | ground, wall, fixture |
| `GetPos`, `GetPosDt`, `GetRot` | 상태 추출 | trajectory, speed, pitch/roll graph |
